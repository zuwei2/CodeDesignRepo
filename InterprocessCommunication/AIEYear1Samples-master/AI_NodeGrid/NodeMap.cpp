...
void NodeMap::Initialise(std::vector<std::string> asciiMap)
{
	...
	for (int y = 0; y < height; y++)
	{
		...
		for (int x = 0; x < width; x++)
		{
			...
			nodes[x + width * y] = tile == emptySquare ? nullptr : new Node(x, y);
		}
	}
	...
	for (int y = 0; y < height; y++)
	{
		for (int x = 0; x < width; x++)
		{
			...
			else
			{
				...
				{
					Node* other = node->connections[i].node;
					DrawLine((x + 0.5f) * cellSize, (y + 0.5f) * cellSize, (other->x + 0.5f) * cellSize, (other->y + 0.5f) * cellSize, lineColor);
				}
			}
		}
	}
